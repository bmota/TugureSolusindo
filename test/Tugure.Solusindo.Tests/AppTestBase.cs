...
using Tugure.Solusindo.Tests.TestDatas;
...
namespace Tugure.Solusindo.Tests
{
    ...
    /// Provides methods to easily work with <see cref="SolusindoDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<SolusindoTestModule>
    {
        ...
        {
            void NormalizeDbContext(SolusindoDbContext context)
            {
        ...
        protected void UsingDbContext(Action<SolusindoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SolusindoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SolusindoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SolusindoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SolusindoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SolusindoDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SolusindoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SolusindoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SolusindoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SolusindoDbContext, Task<T>> func)
        {
            T result;
