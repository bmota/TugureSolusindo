...
using Tugure.Solusindo.Web.IdentityServer;

#if FEATURE_SIGNALR
using Owin;
using Abp.Owin;
using Tugure.Solusindo.Web.Owin;
#endif
...
namespace Tugure.Solusindo.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            //{
            //    options.SwaggerDoc("v1", new Info { Title = "Solusindo API", Version = "v1" });
            ...
            return services.AddAbp<SolusindoWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            //{
            //    options.SwaggerEndpoint("/swagger/v1/swagger.json", "Solusindo API V1");
            //}); //URL: /swagger
        }
        ...
        {
            app.Properties["host.AppName"] = "Solusindo";

            app.UseAbp();

            app.MapSignalR();

            //Enable it to use HangFire dashboard (uncomment only if it's enabled in SolusindoWebCoreModule)
            //app.UseHangfireDashboard("/hangfire", new DashboardOptions
            //{
            //    Authorization = new[] { new AbpHangfireAuthorizationFilter(AppPermissions.Pages_Administration_HangfireDashboard) }
            //});
